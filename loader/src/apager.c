...
int create_elf_tables(struct linux_binprm_lite* bprm, Elf64_Ehdr* exec, unsigned long e_entry) {
    ...
    void* stack = sp;
    // put argc + argv
    *((int*)stack) = argc; stack += 8;
    for (int i = 0; i < argc; i++) {
        *((char**)stack) = argv[i]; stack += 8;
    }
    *((char**)stack) = (char*)NULL; stack += 8;
    // put envp
    for (int i = 0; i < envc; i++) {
        *((char**)stack) = envp[i]; stack += 8;
    }
    *((char**)stack) = (char*)NULL; stack += 8;
    // put auxv
...
int load_elf_binary(struct linux_binprm_lite* bprm, int fd, Elf64_Ehdr* elf_ex) {
    ...
    for (int i = 0; i < phdr_count; i++) {
        ...
        if (elf_brk > elf_bss) {
            if (set_brk(elf_bss, elf_brk, bss_prot) == MAP_FAILED) {
                handle_error("mmap");
            }
        }
        int elf_prot = make_prot(elf_ppnt.p_flags);
        int elf_flags = MAP_PRIVATE | MAP_FIXED | MAP_EXECUTABLE;
        Elf64_Addr vaddr = elf_ppnt.p_vaddr;
        if (elf_map(fd, vaddr, &elf_ppnt, elf_prot, elf_flags) == MAP_FAILED) {
            handle_error("mmap");
        }
...
void init_stack(int argc, char** argv, int envc, char** envp, struct linux_binprm_lite* bprm) {
    ...
    stack_content = end_of_stack;
}

int main(int argc, char** argv, char** envp) {
    if (argc < 2) {
        printf("USAGE: ./apager <filename>");
        return EXIT_FAILURE;
    }
